        ...
        <div className="py-12">
          <header className="text-center mb-12">
            ...
            <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
              2025년 최신 보고서를 기반으로 한 보안 착각과 올바른 인식을 알아보세요.
            </p>
          </header>
                ...
                <div className="border-t border-gray-200">
                  <dl>
                    {selectedCategory.items.map((item, itemIdx) => (
                      <div
                        key={item.id}
                        className={`${itemIdx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6`}>
                        <dt className="text-sm font-medium text-gray-500">
                          <span className="font-bold text-blue-600">착각 {item.id}:</span> {item.misconception}
                        </dt>
